        ...
        <div class="card__body">
            ...
            <div class="course-card__footer">
                <div class="course-progress">
                    ...
                    <span class="progress-text">진행률: ${progress}%</span>
                </div>
...
function switchPracticeTab(tabName) {
    ...
    if (tabName === 'evaluation') {
        ...
        // ROI 계산 버튼이 존재하는지 확인하고 이벤트 리스너 재설정
        setTimeout(() => {
            const roiBtn = document.getElementById('calculateRoi');
            console.log('ROI 버튼 찾기:', roiBtn);
            
            if (roiBtn) {
                console.log('ROI 버튼 발견, 이벤트 리스너 재설정');
                ...
                roiBtn.addEventListener('click', function(e) {
                    console.log('ROI 버튼 직접 클릭됨');
                    e.preventDefault();
                    e.stopPropagation();
                    calculateROI();
                });
                
                console.log('ROI 버튼 이벤트 리스너 재설정 완료');
            } else {
                console.log('ROI 버튼을 찾을 수 없음');
            }
        }, 100);
...
function calculateROI() {
    ...
    function safeParse(id) {
        ...
        if (!el) {
            console.warn(`입력 필드 #${id}를 찾을 수 없습니다.`);
            return 0;
        }
    ...
    try {
        // 상세 비용 입력값들 가져오기
        const costInstructor = safeParse('cost-instructor');
        ...
        // 상세 편익 입력값들 가져오기
        const benefitProductivity = safeParse('benefit-productivity');
...
function calculateROI() {
    ...
    const costSaving = parseFloat(document.getElementById('costSaving').value) || 0;
    ...
    document.getElementById('totalBenefit').textContent = totalBenefit.toLocaleString() + '원';
    document.getElementById('netBenefit').textContent = netBenefit.toLocaleString() + '원';
